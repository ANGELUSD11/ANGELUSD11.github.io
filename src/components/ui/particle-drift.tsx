    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function initParticles() {
                ...
                let baseNodes = Array.from({ length: 90 });
                nodes = baseNodes.slice(0, isMobile ? Math.floor(baseNodes.length * 0.4) : baseNodes.length).map(() => ({
                    x: Math.random() * width,
                ...
                let baseBeams = Array.from({ length: 25 });
                beams = baseBeams.slice(0, isMobile ? Math.floor(baseBeams.length * 0.4) : baseBeams.length).map(() => ({
                    x: Math.random() * width,
                    y: Math.random() * height,
            ...
            function draw() {
                ...
                // Proximity Lines - Optimizadas sin sqrt por defecto
                ctx.lineWidth = 0.5;
                ...
                nodes.forEach(n => {
                    ...
                    let dy = mouse.y - n.y;
                    let distSq = dx * dx + dy * dy;
                    let dist = distSq < 32400 ? Math.sqrt(distSq) : 9999;
...
const PARTICLE_DRIFT_DEFINITION: EffectDefinition = {
  ...
  patch(source, { size, length, density, mode }) {
    ...
    let next = source
      ...
      .replace("if(distSq < 14400) {", `if(distSq < ${linkSq}) {`)
